...
if  ($musername != "" || $_SESSION['apassword'] == $adminpassword) {
	...
	if (isset($email) && $email != "" && $counting != 0) { 
		...
		$stmt = $dbconnect->prepare("UPDATE members SET email = '$email' WHERE username = '$username'");
...
if ($updatepassword == "" || $updatepassword2 == "" ) {
		...
		$stmt = $dbconnect->prepare("UPDATE members SET password = '$hash' WHERE username = '$username'");
...
if (isset($profilephoto)) {
			...
			$totalfilesprocessed = count($names);
		 	...
		 	if ($totalfilesuploaded != $totalfilesprocessed) {
		 			...
		 			$stmt = $dbconnect->prepare("UPDATE members SET photo = '$name' WHERE username = '$username'");
					$stmt->bind_param("s", $name);
					$stmt->execute();

					$stmt2 = $dbconnect->prepare("UPDATE members SET mimetype = '$mimetype' WHERE username = '$username'");
					$stmt2->bind_param("s", $mimetype);
					$stmt2->execute();

					$stmt3 = $dbconnect->prepare("UPDATE members SET photoname = '$photoname' WHERE username = '$username'");
